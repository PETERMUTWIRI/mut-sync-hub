...
export function middleware(req: NextRequest) {
  // 1. quick JWT existence check – no DB, no client
  const stackJWT = req.cookies.get('stack-session')?.value;
  if (!stackJWT) {
    return NextResponse.redirect(new URL('/sign-in', req.url));
  }

  // 2. TODO: after 404 is gone, move 2-factor check into a *server* action
  //          instead of doing it here in edge.
  return NextResponse.next();
}

export const config = {
  matcher: ['/app/:path*'], // keep your original matcher
};